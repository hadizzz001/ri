import Link from "next/link";

export default function Features() {
    return (
        <>
            <div className="sywPaymentOptions">




                <style
                    dangerouslySetInnerHTML={{
                        __html:
                            "\n    .border-gray-400 {\n    --tw-border-opacity: 1;\n    border-color: rgb(156 163 175 / var(--tw-border-opacity)) !important;\n}\n"
                    }}
                />

                <div className="relative flex py-5 items-center">
                    <div className="flex-grow " />
                    <span className="flex-shrink mx-4  ">
                        <h1 className="uppercase" style={{fontSize:'40px'}}>Rafi Daham</h1>
                        <h1 className="uppercase" style={{fontSize:'40px'}}>Hair Repair Studio</h1>
                        </span>
                    <div className="flex-grow" />
                </div>


                <p className="text-lg  text-center">Rafi Daham embarked on his hairstyling journey in 1994, transforming his passion for artistry and precision into a lifelong career. With each snip and stroke, he honed his craft, blending creativity with technique to shape stunning looks that reflect both individuality and style.</p>

            </div>

        </>
    )
}